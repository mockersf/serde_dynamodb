...
fn impl_build_query_input(name: &Ident, visibility: &Visibility, variant_data: VariantData, ty_params: &[TyParam]) -> quote::Tokens {

    let types = get_type_params(ty_params);
    let type_params = quote! { <#(#types)*> };
    let fields_as_options: Vec<Field> = variant_data.fields().iter().map(|field| {

        // wrap type in an option to allow filtering only on some fields
        let original_type: &syn::Ty = &field.ty;
        let optional_type = Ty::Path(None, syn::Path {
            global: false,
            segments: vec![
                syn::PathSegment {
        ...
        let mut renamed_attributes: Vec<syn::Attribute> = field.attrs.iter().map(|meta_items| { 
            let new_value = match meta_items.clone().value {
                List(ref name, ref items) if name == "serde" => {
                    let mut new_items: Vec<syn::NestedMetaItem> = items.iter().cloned().map(|meta_item| {
                        match meta_item {
                            MetaItem(NameValue(ref name, ref lit)) if name == "rename" => 
                                MetaItem(NameValue("rename".into(), format!(":{}", get_string_from_lit(name.as_ref(), name.as_ref(), lit)).into())),
                            other => other,
                        }
                    }).collect();
                    if new_items.iter().cloned().filter(|meta_item| match *meta_item {
                        MetaItem(NameValue(ref name, _)) if name == "rename" => true,
                        _ => false,
                    }).count() == 0 {
                        new_items.push(
                            MetaItem(NameValue("rename".into(), format!(":{}", field.ident.clone().unwrap()).into()))
                        );
                    }
        ...
        if renamed_attributes.iter().cloned().filter(|meta_items| match meta_items.value {
            List(ref name, _) if name == "serde" => true,
            _ => false
        }).count() == 0 {
            renamed_attributes.push(
                Attribute {
                    value: List("serde".into(), vec![MetaItem(NameValue("rename".into(), format!(":{}", field.ident.clone().unwrap()).into()))]),
    ...
    let fields_as_conditions: Vec<quote::Tokens> = variant_data.fields().iter().map(|field| {
